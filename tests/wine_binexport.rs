...
fn test_headless() {
    ...
    assert!(ida.is_headless());
}


#[test]
fn test_wine_binexport() {
    let mut settings = config::Config::default();
    // safe to panic here with unwrap since it's a test
    settings
        .merge(config::File::with_name("TestConfig"))
    ...
    let script_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("ida_binexport.py");

    let out = conf.get("binexport_dump").expect("no binexport output dump path specified");
